...
public class MedicalFormActivity extends AppCompatActivity {
    ...
    private AutoCompleteTextView spinnerResident;
                ...
                if (isEditMode && medical != null) {
                    for (int i = 0; i < residentList.size(); i++) {
                        if (residentList.get(i).getId() == medical.getResidentId()) {
                            spinnerResident.setText(residentNames.get(i), false);
                            break;
                        }
                    }
                }
    ...
    private void populateFormData() {
        if (medical != null) {
            ...
            if (residentList != null && !residentList.isEmpty()) {
                for (int i = 0; i < residentList.size(); i++) {
                    if (residentList.get(i).getId() == medical.getResidentId()) {
                        spinnerResident.setText(residentList.get(i).getName(), false);
                        break;
                    }
                }
            }
    ...
    private void saveMedicalRecord() {
        if (validateForm()) {
            ...
            // 正确设置居民ID
            if (residentList != null && !residentList.isEmpty()) {
                String selectedResidentName = spinnerResident.getText().toString();
                boolean found = false;
                for (int i = 0; i < residentList.size(); i++) {
                    if (residentList.get(i).getName().equals(selectedResidentName)) {
                        record.setResidentId(residentList.get(i).getId());
                        found = true;
                        break;
                    }
                }
    ...
    private boolean validateForm() {
        if (spinnerResident.getText().toString().isEmpty()) {
            Toast.makeText(this, "请选择居民", Toast.LENGTH_SHORT).show();
            return false;
        }

        if (etLastCheckupDate != null && etLastCheckupDate.getText().toString().trim().isEmpty()) {
            Toast.makeText(this, "请选择最后体检日期", Toast.LENGTH_SHORT).show();
            return false;
        }
